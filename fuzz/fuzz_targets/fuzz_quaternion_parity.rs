...
use dew_core::Expr;
use dew_quaternion::{quaternion_registry, cranelift::{QuaternionJit, VarSpec}, lua::eval_lua, Value};
...
fuzz_target!(|input: QuaternionParityInput| {
    ...
    let Ok(eval_val) = dew_quaternion::eval(expr.ast(), &var_map, &registry) else {
        return;
    };
