...
//! # #[cfg(feature = "linalg")]
//! # {
//! use dew_linalg::{eval, linalg_registry, LinalgValue};
//!
...
//! let mut registry = dew_linalg::FunctionRegistry::new();
//! dew_linalg::register_linalg(&mut registry);
//!
...
#[cfg(feature = "linalg")]
impl<T: Float + dew_core::Numeric> dew_linalg::LinalgValue<T> for Value<T> {
    fn typ(&self) -> dew_linalg::Type {
        match self {
            Value::Scalar(_) => dew_linalg::Type::Scalar,
            Value::Vec2(_) => dew_linalg::Type::Vec2,
            Value::Vec3(_) => dew_linalg::Type::Vec3,
            Value::Vec4(_) => dew_linalg::Type::Vec4,
            Value::Mat2(_) => dew_linalg::Type::Mat2,
            Value::Mat3(_) => dew_linalg::Type::Mat3,
            Value::Mat4(_) => dew_linalg::Type::Mat4,
            _ => dew_linalg::Type::Scalar, // fallback for non-linalg types
        }
    }
...
#[cfg(feature = "complex")]
impl<T: Float + std::fmt::Debug> dew_complex::ComplexValue<T> for Value<T> {
    fn typ(&self) -> dew_complex::Type {
        match self {
            Value::Scalar(_) => dew_complex::Type::Scalar,
            Value::Complex(_) => dew_complex::Type::Complex,
            _ => dew_complex::Type::Scalar, // fallback for non-complex types
        }
    }
...
#[cfg(feature = "quaternion")]
impl<T: Float + std::fmt::Debug> dew_quaternion::QuaternionValue<T> for Value<T> {
    fn typ(&self) -> dew_quaternion::Type {
        match self {
            Value::Scalar(_) => dew_quaternion::Type::Scalar,
            Value::Vec3(_) => dew_quaternion::Type::Vec3,
            Value::Quaternion(_) => dew_quaternion::Type::Quaternion,
            _ => dew_quaternion::Type::Scalar, // fallback for non-quaternion types
        }
    }
...
mod tests {
    ...
    fn test_linalg_eval_with_combined_value() {
        ...
        let result = dew_linalg::eval(expr.ast(), &vars, &registry).unwrap();
        assert_eq!(result, Value::Scalar(1.0));
    }
    ...
    fn test_complex_eval_with_combined_value() {
        ...
        let mut registry = dew_complex::FunctionRegistry::new();
        dew_complex::register_complex(&mut registry);

        let result = dew_complex::eval(expr.ast(), &vars, &registry).unwrap();
    ...
    fn test_quaternion_eval_with_combined_value() {
        ...
        let mut registry = dew_quaternion::FunctionRegistry::new();
        dew_quaternion::register_quaternion(&mut registry);

        let result = dew_quaternion::eval(expr.ast(), &vars, &registry).unwrap();
