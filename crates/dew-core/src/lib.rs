...
//! use dew_core::{Expr, ExprFn, FunctionRegistry, Ast};
//! use std::collections::HashMap;
//!
//! struct Clamp;
...
/// # #[cfg(feature = "func")]
/// # assert_eq!(expr.eval(&vars, &dew_core::FunctionRegistry::new()).unwrap(), 16.0);
///
/// vars.insert("x".to_string(), 0.0);
/// # #[cfg(not(feature = "func"))]
/// assert_eq!(expr.eval(&vars).unwrap(), 1.0);   // 0 + 0 + 1
/// # #[cfg(feature = "func")]
/// # assert_eq!(expr.eval(&vars, &dew_core::FunctionRegistry::new()).unwrap(), 1.0);
/// ```
#[derive(Debug, Clone)]
pub struct Expr {
    ast: Ast,
